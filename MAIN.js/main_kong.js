...
socket.on('KongConnect', function( result_Path, Tile_Path, dates, GeoJSON_fileNames, TILE_fileNames) {
    ...
    function processGeoJSONFile(filePath, fileName) {
        ...
        if (fileName.toLowerCase().includes("ais")) {           // fileName 有包含 "ais" 的檔案 就丟入 jQuery 的 getJSON
            $.getJSON(filePath, function (data) {               // 檔案丟入 jQuery 的 getJSON, 得到的data 要做什麼處理 寫在 第二個參數的function內
                ...
                var boat_geoJSON = L.geoJSON(data, {          
                    // pointToLayer, 特點是 會把 data裡面的 每筆feature 和 feature的經緯度 return 給user使用  
                    pointToLayer: function (feature, latlng) {
                        // 得到的 每筆 feature 和 feature的經緯度 要做什麼寫這裡
                        // 使用自定義圖標創建標記
                ...
                // leaflet圖層 根據檔名中的日期 加入 boatLayers_AIS 物件 裡面
                boat_geoJSON.addTo(boatLayers_AIS[`${fileName.substring(0, 8)}`]);
            });
        }
        else if (fileName.toLowerCase().includes("match")) {    // fileName 有包含 "match" 的檔案 就丟入 jQuery 的 getJSON
            $.getJSON(filePath, function (data) {               // 檔案丟入 jQuery 的 getJSON, 得到的data 要做什麼處理 寫在 第二個參數的function內
                // 建立Leaflet圖層(用 得到的 data 要拿來建), 使用 L.getJSON來建立圖層, 第一個參數放data, 第二個參數放 處理data的 物件, 物件長相可參考 https://leafletjs.com/examples/geojson/
                var boat_geoJSON = L.geoJSON(data, {
                ...
                // leaflet圖層 根據檔名中的日期 加入 boatLayers_MATCH 物件 裡面
                boat_geoJSON.addTo(boatLayers_MATCH[`${fileName.substring(0, 8)}`]);
            });
        }
        // ##########################2023/12/13新增(頭)##########################
        else if (fileName.toLowerCase().includes("area")) {     // fileName 有包含 "area" 的檔案 就丟入 jQuery 的 getJSON
            $.getJSON(filePath, function (data) {               // 檔案丟入 jQuery 的 getJSON, 得到的data 要做什麼處理 寫在 第二個參數的function內
                // 建立Leaflet圖層(用 得到的 data 要拿來建), 使用 L.getJSON來建立圖層, 第一個參數放data, 第二個參數放 處理data的 物件, 物件長相可參考 https://leafletjs.com/examples/geojson/
                var boat_geoJSON = L.geoJSON(data, {          
                    // onEachFeature, 特點是 會把 data裡面的 每筆feature 和 layer return 給 user使用, 這裡 layer用不到 只有用到 feature這樣子喔～  
                ...
                // leaflet圖層 根據檔名中的日期 加入 boatLayers_AREA 物件 裡面
        ...
        // ##########################2023/12/13新增(尾)##########################
            ...
            $.getJSON(filePath, function (data) {
                ...
                // leaflet圖層 根據檔名中的日期 加入 boatLayers 物件 裡面
...
//client 傳訊息
socket.emit("ClientConnect", "客戶端回傳資訊囉");
